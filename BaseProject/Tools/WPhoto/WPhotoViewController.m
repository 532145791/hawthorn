...
- (void)getHeightVersionAllPhotos {
    ...
    [WPFunctionView getHeightVersionAllPhotos:^(PHFetchResult *allPhotos) {
        ...
        if (!weakSelf.allPhotoArr) {
            weakSelf.allPhotoArr = [[NSMutableArray alloc]init];
        }
        ...
        for (NSInteger i = 0; i < allPhotos.count; i++) {
            ...
            if (asset.mediaType == PHAssetMediaTypeImage) {
                [weakSelf.allPhotoArr addObject:asset];
            }
            
            NSString *cellId = [NSString stringWithFormat:@"cell%ld", (long)i];
            [weakSelf.ado_collectionView registerClass:[myPhotoCell class] forCellWithReuseIdentifier:cellId];
            
        }
        [weakSelf.ado_collectionView reloadData];
    }];
}
...
- (void)getLowVersionAllPhotos {
    ...
    [WPFunctionView getLowVersionAllPhotos:^(ALAssetsGroup *group) {
        if (!weakSelf.photoGroupArr) {
            weakSelf.photoGroupArr = [[NSMutableArray alloc]init];
        }
        
        if (group!=nil) {
            [weakSelf.photoGroupArr addObject:group];
        }
        else{
            ALAssetsGroup* allPhotoGroup = weakSelf.photoGroupArr[weakSelf.photoGroupArr.count-1];
            ...
            [allPhotoGroup enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
                ...
                if (result&&[[result valueForProperty:ALAssetPropertyType]isEqualToString:ALAssetTypePhoto]) {
                    ...
                    //照片内容url加入数组
                    [weakSelf.allPhotoArr addObject:result.defaultRepresentation.url];
                }
                else{
                    //刷新显示
                    if (weakSelf.allPhotoArr.count) {
                        for (NSInteger i = 0; i<weakSelf.allPhotoArr.count; i++) {
                            NSString *cellId = [NSString stringWithFormat:@"cell%ld", (long)i];
                            [weakSelf.ado_collectionView registerClass:[myPhotoCell class] forCellWithReuseIdentifier:cellId];
                        }
                        [weakSelf.ado_collectionView reloadData];
        ...
        [WPFunctionView getChoosePicPHImageManager:^(double progress) {
            ...
            if (!result) {
                ...
                if (cell.chooseStatus == NO) {
                    if ((weakSelf.chooseArray.count+weakSelf.choosePhotoArr.count)< weakSelf.selectPhotoOfMax) {
                        [weakSelf.chooseArray addObject:result];
                        [weakSelf.chooseCellArray addObject:[NSString stringWithFormat:@"%ld",(long)indexPath.row]];
                        [self finishColorAndTextChange:weakSelf.chooseArray.count+weakSelf.choosePhotoArr.count];
                    ...
                    for (NSInteger j = 0; j<weakSelf.chooseCellArray.count; j++) {
                        
                        NSIndexPath *ip = [NSIndexPath indexPathForRow:[weakSelf.chooseCellArray[j] integerValue] inSection:0];
                        
                        if (indexPath.row == ip.row) {
                            [weakSelf.chooseArray removeObjectAtIndex:j];
                        }
                    }
                    [weakSelf.chooseArray removeObject:result];
                    [weakSelf.chooseCellArray removeObject:[NSString stringWithFormat:@"%ld",(long)indexPath.row]];
                    [weakSelf finishColorAndTextChange:weakSelf.chooseArray.count+weakSelf.choosePhotoArr.count];
...
-(void)finishChoosePhotos:(UIButton *)finishbtn{
    ...
    if (![finishbtn.titleLabel.text isEqualToString:finishStr]&&_chooseArray.count) {
        ...
        [WPFunctionView finishChoosePhotos:^(NSMutableArray *myChoosePhotoArr) {
            weakSelf.selectPhotosBack(myChoosePhotoArr);
