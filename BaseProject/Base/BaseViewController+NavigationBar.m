...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self.navigationController.navigationBar setTitleTextAttributes:
     @{NSFontAttributeName:(self.confDic[kNav_TitleFont] ? self.confDic[kNav_TitleFont] : Font_Medium(17)),
       NSForegroundColorAttributeName:(self.confDic[kNav_TitleColor] ? self.confDic[kNav_TitleColor] : Color(kColor_333333))}];
}
...
- (void)resetNavigationItemWithLeftTitle:(NSString *)title font:(UIFont *)font titleColor:(UIColor *)titleColor
{
    if(title && title.length > 0){
        ...
        [self.navigationItem.leftBarButtonItem setTitleTextAttributes:@{NSFontAttributeName:(font ? font : Font_Regular(15)),
                                                                        NSForegroundColorAttributeName:(titleColor ? titleColor : Color(kColor_333333))} forState:UIControlStateNormal];
    }
}
...
- (NSString *)navImageNameWithType:(MSNavBarItemType)navType{
    ...
    switch (navType) {
        case NavBarItemTypeBack:
            imageName = @"nav-black-back";
            break;
        case NavBarItemTypeHelp:
            imageName = @"hp_function_wenhao";
            break;
        case NavBarItemTypeShare:
            imageName = @"homepage_share_icon_gray";
            break;
